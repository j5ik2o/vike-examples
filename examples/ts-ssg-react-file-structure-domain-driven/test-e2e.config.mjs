...
function tolerateError({ logSource, logText }) {
  ...
  function isServiceExit() {
    return (
      logSource === "stderr" &&
      (logText.includes("The service is no longer running") ||
        logText.includes("The service was stopped"))
    );
  }
  ...
  function isCloudflareFalseError2() {
    return (
      logSource === "stderr" &&
      logText.includes("Script modified; context reset.")
    );
  }
  ...
  function isCloudflarePrewarmWarning() {
    return (
      logSource === "stderr" && logText.includes("worker failed to prewarm")
    );
  }
  ...
  function isSlowCrawlWarning() {
    return (
      logSource === "stderr" &&
      logText.includes("Crawling your user files took an unexpected long time")
    );
  }
