...
function testRun(cmd: "npm run dev" | "npm run preview", isV1Design?: true) {
  ...
  test("HTML-only", async () => {
    ...
    if (isPreview) {
      ...
      expect(html).toContain(
        '<link rel="stylesheet" type="text/css" href="/pages/html-only/index.css?direct">',
      );
    }
    ...
    test("HTML-only - HMR", async () => {
      {
        ...
        // No HMR for JavaScript
        {
          ...
          const file = isV1Design
            ? "./pages/html-only/+Page.jsx"
            : "./pages/html-only/index.page.server.jsx";
          editFile(file, (s) =>
      ...
      {
        await testColor("orange");
        editFile("./pages/html-only/index.css", (s) =>
          s.replace("color: orange", "color: gray"),
        );
    ...
    if (!isPreview) {
      {
        ...
        const file = isV1Design
          ? "./pages/spa/+Page.jsx"
          : "./pages/spa/index.page.client.jsx";
      ...
      expect(await page.textContent("button")).toContain("Counter 1");
      {
        ...
        await sleep(HMR_SLEEP);
        editFile("./pages/spa/index.css", (s) =>
          s.replace("color: green", "color: gray"),
        );
        await testColor("gray");
        await sleep(HMR_SLEEP);
        editFileRevert();
    ...
    test("HTML + JS - HMR", async () => {
      ...
      // JS auto-reload
      {
        expect(await page.textContent("h1")).toBe("HTML + JS");
        {
          ...
          const navPromise = page.waitForNavigation();
          const file = isV1Design
            ? "./pages/html-js/+Page.jsx"
            : "./pages/html-js/index.page.server.jsx";
          editFile(file, (s) =>
      ...
      {
        await testColor("red");
        editFile("./pages/html-js/index.css", (s) =>
          s.replace("color: red", "color: gray"),
        );
        await testColor("gray");
        editFileRevert();
        await testColor("red");
  ...
  test("SSR", async () => {
    ...
    if (!isPreview) {
      ...
      expect(await page.textContent("button")).toContain("Counter 1");
      {
        ...
        await sleep(HMR_SLEEP);
        editFile("./pages/ssr/index.css", (s) =>
          s.replace("color: blue", "color: gray"),
        );
        await testColor("gray");
        await sleep(HMR_SLEEP);
        editFileRevert();
  ...
  test("CSS of other pages isn't loaded", async () => {
    {
      ...
      if (!isPreview) {
        expect(html).toContain(
          '<link rel="stylesheet" type="text/css" href="/renderer/PageLayout.css',
        );
      }
    }
    for (const page of ["html-only", "html-js", "spa", "ssr"]) {
      ...
      if (!isPreview) {
        ...
        expect(html).toContain(
          `<link rel="stylesheet" type="text/css" href="/pages/${page}/index.css`,
        );
      }
    }
  });
  ...
  async function clickCounter() {
    await page.waitForFunction(() =>
      window.document.body.textContent!.includes("Counter"),
